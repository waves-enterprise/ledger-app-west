...
def test_status_word(main_h_path):
    expected_status_words: List[Tuple[str, int]] = parse_sw(main_h_path)
    ...
    for sw in status_words.keys():
        assert sw in expected_status_words, f"{status_words[sw]}({hex(sw)}) not found in main.h!"
