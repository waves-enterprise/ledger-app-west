...
void hash_stream_data(uint8_t chunk_data_size, uint8_t chunk_data_start_index) {
    ...
    stream_eddsa_sign_step2(
        &tmp_ctx.signing_context.eddsa_context,
        &G_io_apdu_buffer[chunk_data_start_index + tmp_ctx.signing_context.chunk_used],
...
void make_sign_step(uint8_t chunk_data_start_index, uint8_t chunk_data_size) {
    ...
    if (tmp_ctx.signing_context.step == 1) {
        ...
        if (tmp_ctx.signing_context.data_read < tmp_ctx.signing_context.data_size) {
            ...
            if (tmp_ctx.signing_context.data_read == tmp_ctx.signing_context.data_size) {
                memmove(ui_context.id, tmp_ctx.signing_context.eddsa_context.first_data_hash, 32);
...
uint32_t set_result_get_app_version() {
...
uint32_t set_result_get_app_name() {
    memmove((char *) G_io_apdu_buffer, APPNAME, APPNAME_LEN);
    return APPNAME_LEN;
}
        ...
        TRY {
            ...
            switch (G_io_apdu_buffer[1]) {
                ...
                case INS_GET_APP_VERSION:
                    *tx = set_result_get_app_version();
                    THROW(SW_OK);
                    break;

                case INS_GET_APP_NAME:
                    *tx = set_result_get_app_name();
                    THROW(SW_OK);
                    break;
