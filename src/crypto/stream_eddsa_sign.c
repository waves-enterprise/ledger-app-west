...
static uint8_t const C_cx_Ed25519_a[] = { 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
static uint8_t const C_cx_Ed25519_Bx[] = { 0x21, 0x69, 0x36, 0xd3, 0xcd, 0x6e, 0x53, 0xfe,
                                           0xc0, 0xa4, 0xe2, 0x31, 0xfd, 0xd6, 0xdc, 0x5c,
                                           0x69, 0x2c, 0xc7, 0x60, 0x95, 0x25, 0xa7, 0xb2,
                                           0xc9, 0x56, 0x2d, 0x60, 0x8f, 0x25, 0xd5, 0x1a };
static uint8_t const C_cx_Ed25519_By[] = { 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66,
...
static uint8_t const C_cx_Ed25519_h[] = {
    // h: 8
    0x08
};
...
/*                                                                         */
/* ----------------------------------------------------------------------- */
static void cx_eddsa_get_public_key_internal(const cx_ecfp_private_key_t *pv_key,
                                             cx_md_t hashID,
                                             cx_ecfp_public_key_t *pu_key,
                                             unsigned char *a,
                                             __attribute__((unused)) unsigned int a_len,
                                             unsigned char *h,
                                             __attribute__((unused)) unsigned int h_len,
                                             unsigned char *scal /*tmp*/) {
    cx_curve_twisted_edwards_t *domain = (cx_curve_twisted_edwards_t *) &C_cx_Ed25519;
...
void stream_eddsa_sign_step1(streamEddsaContext_t *eddsa_context,
    ...
    cx_eddsa_get_public_key_internal(pv_key,
                                     CX_SHA512,
                                     (cx_ecfp_public_key_t *) &eddsa_context->u.internal_pu_key,
                                     eddsa_context->a,
                                     sizeof(eddsa_context->a),
                                     eddsa_context->r,
                                     sizeof(eddsa_context->r),
                                     scal);

    // compress public_key
    CX_THROW(cx_edwards_compress_point_no_throw(domain->curve,
...
void stream_eddsa_sign_step3(streamEddsaContext_t *eddsa_context) {
    ...
    cx_hash(&eddsa_context->data_hash_ctx.header,
            CX_LAST,
            NULL,
            0,
            eddsa_context->first_data_hash,
            32);
    ...
    cx_ecfp_scalar_mult(CX_CURVE_Ed25519,
                        eddsa_context->u.Q,
                        1 + 2 * domain->length,
                        eddsa_context->r,
                        size);
...
int stream_eddsa_sign_step5(streamEddsaContext_t *eddsa_context, unsigned char *sig) {
    ...
    if (memcmp(&eddsa_context->first_data_hash, &second_data_hash, 32) != 0) {
    ...
    cx_math_addm(eddsa_context->sig + size,
                 eddsa_context->sig + size,
                 eddsa_context->r,
                 domain->n,
                 size);
