...
void try_to_fill_buffer(uint8_t chunk_data_start_index, uint8_t chunk_data_size) {
    ...
    memmove((unsigned char *) &ui_context.buffer[ui_context.buffer_used],
...
void build_other_data_ui() {
    ...
    if (tx_type == 3) {
        memmove(&ui_context.line1, &"issue\0", 6);
    } else if (tx_type == 4) {
        memmove(&ui_context.line1, &"transfer\0", 9);
    } else if (tx_type == 5) {
        memmove(&ui_context.line1, &"reissue\0", 8);
    } else if (tx_type == 6) {
        memmove(&ui_context.line1, &"burn\0", 5);
    } else if (tx_type == 8) {
        memmove(&ui_context.line1, &"start leasing\0", 14);
    } else if (tx_type == 9) {
        memmove(&ui_context.line1, &"cancel leasing\0", 15);
    } else if (tx_type == 10) {
        memmove(&ui_context.line2, &"Transaction Hash\0", 17);
        memmove(&ui_context.line1, &"creating an alias\0", 18);
    } else if (tx_type == 11) {
        ...
        memmove(&ui_context.line1, &"sponsorship\0", 12);
    } else if (tx_type == 15) {
        memmove(&ui_context.line1, &"asset script\0", 13);
    } else if (tx_type == 16) {
        memmove(&ui_context.line1, &"script invocation\0", 18);
    } else if (tx_type == 17) {
        memmove(&ui_context.line1, &"update asset info\0", 18);
    } else if (tx_type > 200) {
        ...
        if (tx_type == 252) {
            ...
            memmove(&ui_context.line1, &"something\0", 10);
        }
    }
    ...
    size_t length = 45;
    if (!b58enc((char *) ui_context.line3, &length, (const void *) &ui_context.id, 32)) {
    ...
    if (!get_curve25519_public_key_for_path((uint32_t *) tmp_ctx.signing_context.bip32,
                                            &public_key)) {
        THROW(INVALID_PARAMETER);
    }

    waves_public_key_to_address(public_key.W,
