...
def get_keys_from_dongle(path, networkByte):
    global dongle
    while True:
        try:
            data_bytes = bytes(bytearray.fromhex("800400" + '{0:x}'.format(ord(networkByte)) + "14"))
            data_bytes += path_to_bytes(path)
            ...
            input(e)
            input("An error occurred while processing the request, repeat or correct your request (note what all the "
                  "bip32 path parts should be hardened)")
            sys.exc_clear()
            break


def get_version_from_dongle():
    global dongle
    while True:
        try:
            data_bytes = bytes(bytearray.fromhex("8006000000"))
            data = dongle.exchange(data_bytes)
            return data[0:3]
        except CommException as e:
            if e.sw == 0x6985:
...
while (True):
    ...
    select = input(colors.fg.cyan + "Please select> " + colors.reset)

    if select == "1":
        path = input(
            colors.fg.lightblue + "Please input BIP-32 path (for example \"44'/1'/0'/0'/1'\")> " + colors.reset)
        if len(path) == 0:
            path = "44'/1'/0'/0'/1'"
        keys = get_keys_from_dongle(expand_path(path), chain_id)
        if keys:
            publicKey = keys[0]
            address = keys[1]

            print(colors.fg.blue + "publicKey (base58): " + colors.reset + base58.b58encode(publicKey).decode())
            print(colors.fg.blue + "address: " + colors.reset + address.decode())
    elif select == "2":
        path = input(
            colors.fg.lightblue + "Please input BIP-32 path (for example \"44'/1'/0'/0'/1'\")> " + colors.reset)
        if len(path) == 0:
        ...
        print(colors.fg.lightgrey + "path bytes: " + binary_data.hex())
        ...
        some_transfer_bytes = build_transfer_bytes('6zejiDoUzapfdjWuXyfqZfv4FPshGndetV9TyutRF9xD',
                                                   pw.Address('3PMpANFyKGBwzvv1UVk2KdN23fJZ8sXSVEK'),
                                                   0,
                                                   1,
                                                   '',
                                                   timestamp=1526477921829)
        input_data = input(colors.fg.lightblue + "Please input message to sign (for example \"" + base58.b58encode(
            some_transfer_bytes).decode() + "\")> " + colors.reset)
        if len(input_data) == 0:
            ...
            binary_data += some_transfer_bytes
            print(colors.fg.lightgrey + "tx bytes:   " + some_transfer_bytes.hex())
        else:
            binary_input = base58.b58decode(input_data)
            binary_data += struct.pack(">I", len(binary_input))
            binary_data += binary_input
            binary_data += binary_input
            print(colors.fg.lightgrey + "tx bytes:   " + binary_input.hex())
        print(colors.fg.lightgrey + "all request bytes:   " + binary_data.hex())
        signature = None
        while True:
            try:
                offset = 0
                while offset != len(binary_data):
                    ...
                    apdu = bytes(bytearray.fromhex("8002")) + p1 + bytes(chain_id + chr(len(chunk)),
                                                                         encoding='utf-8') + chunk
                    # print(apdu.hex())
                    signature = dongle.exchange(apdu)
                    offset += len(chunk)
                print("signature " + base58.b58encode(signature).decode())
                ...
                print(e, type(e))
                answer = input("Please connect your Ledger Nano S, unlock, and launch the Waves app. Press <enter> "
                               "when ready. (Q quits)")
                if answer.upper() == 'Q':
                    sys.exit(0)
                sys.exc_clear()
    elif select == "3":
        version = get_version_from_dongle()
